...
class Movietopicimg extends CI_Model {
     ...
     * @return mixed
     */
    public function getTopicMovieImgByRelatedId($relatedId = array(),$type = 1)
    {
        $relatedId = array_unique($relatedId);
        if (empty($relatedId) || !is_array($relatedId)) {
            return array();
        }
        $sql = "select * from `tbl_movieTopicImg` where relatedId in (" . implode(",",$relatedId) . ") and type = ? and del = 0;";
