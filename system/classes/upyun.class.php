...
/*}}}*/
	...
	*/
    ...
    /**
     * 上传图片，added by xiongjiewu at 2013-08-17
     * @param $imgPath 图片路径
     * @param $serverFile 上传的图片路径
     * @param bool $auto_mkdir 是否自动创建文件夹
     * @param null $opts 可选参数
     * @return bool
     */
    public function putImg($imgPath,$serverFile,$auto_mkdir = False, $opts = NULL) {
        $fn = file_get_contents($imgPath);
        $up = $this->writeFile($serverFile,$fn,$auto_mkdir,$opts);
        if (empty($up)) {
            return false;
        } else {
            return $serverFile;
