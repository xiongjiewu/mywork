<?php
define("BASEPATH", "runJob");
if (is_dir("/home/jie/projects/other/aa/application/")) {
    define("APPPATH","/home/jie/projects/other/aa/application/");
    define("FILEPATH",dirname(__FILE__));
    define("IMGPATH","/home/jie/projects/other/img/");
} else {
    define("APPPATH","/home/jie/projects/admin/application/");
    define("FILEPATH",dirname(__FILE__));
    define("IMGPATH","/home/jie/projects/img/");
}

include(FILEPATH . "/../../system/classes/upyun.class.php");
...
class myPdo extends PDO
{
    ...
    function getPdo()
    {
        $database = $this->_configPath . "database.php";
        include(FILEPATH . "/" .$database);
        $dsn = "mysql:host={$db['default']['hostname']};dbname={$db['default']['database']}";
        $db = new PDO($dsn, $db['default']['username'], $db['default']['password']);
    ...
    public function get_config_value($name,$file = "common") {
        if (!class_exists("APF")) {
            if (defined("APPPATH")) {
                $apfPath = APPPATH . "../system/classes/apf.php";
            } else {
                $apfPath = FILEPATH . "/../../system/classes/apf.php";
            }
            include($apfPath);
        }
        return APF::get_instance()->get_config_value($name,$file);
    }
