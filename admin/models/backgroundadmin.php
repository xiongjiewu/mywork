...
class Backgroundadmin extends CI_Model {
    ...
    public function getDetailInfoBySearchW($searchW,$limit=10,$del = 0)
    {
        if (!isset($searchW)) {
            return false;
        }
        $sql = "select {$this->_getFiledStr()} from tbl_detailInfo where name like '{$searchW}%' and del = {$del} limit {$limit};";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
